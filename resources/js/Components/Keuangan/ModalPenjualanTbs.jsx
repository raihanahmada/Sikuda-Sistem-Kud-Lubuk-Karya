        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
                ...
                <form onSubmit={simpan} className="space-y-4">
                    ...
                    <div>
                        <label className="mb-1 block text-sm font-medium text-gray-700">Berat Bersih (kg)</label>
                        <input type="number" step="0.01" min="0.01" value={form.data.berat_bersih_kg}
                            ...
                            className={`w-full rounded-md border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-600 ${
                                form.errors.berat_bersih_kg ? 'border-red-400 bg-red-50' : 'border-gray-300'
                            }`} />
                        {form.errors.berat_bersih_kg && <p className="mt-1 text-xs text-red-600">{form.errors.berat_bersih_kg}</p>}
                    </div>
                    ...
                    <div className="rounded-lg bg-gray-50 px-3 py-2 text-sm">
                        <div className="flex justify-between">
                            ...
                            <span className="text-gray-700">{rupiah(hargaDipakai)}</span>
                        </div>
                    ...
                    <div>
                        <label className="mb-1 block text-sm font-medium text-gray-700">Tanggal Timbang</label>
                        <input type="date" value={form.data.tanggal_timbang}
                            onChange={(e) => form.setData('tanggal_timbang', e.target.value)}
                            className={`w-full rounded-md border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-600 ${
                                form.errors.tanggal_timbang ? 'border-red-400 bg-red-50' : 'border-gray-300'
                            }`} />
                        {form.errors.tanggal_timbang && <p className="mt-1 text-xs text-red-600">{form.errors.tanggal_timbang}</p>}
                    </div>
