                ...
                <form onSubmit={simpan} className="space-y-4">
                    ...
                    {edit ? (
                        <div className="rounded-lg bg-gray-50 px-3 py-2 text-sm">
                            ...
                            <span className="font-medium text-gray-900">{data.nama_anggota}</span>
                            <p className="mt-0.5 text-[11px] text-gray-400">Penjualan asal tidak dapat diubah saat edit</p>
                        </div>
                    ) : (
                        <div>
                            <label className="mb-1 block text-sm font-medium text-gray-700">Penjualan TBS</label>
                            <SearchableSelect
                            ...
                            />
                            {form.errors.id_penjualan && <p className="mt-1 text-xs text-red-600">{form.errors.id_penjualan}</p>}
                        </div>
                    )}

                    {/* Total penjualan (read-only) */}
                    <div className="flex justify-between rounded-lg bg-gray-50 px-3 py-2 text-sm">
                        <span className="text-gray-500">Total Penjualan TBS:</span>
                        <span className="font-semibold text-gray-900">{rupiah(totalPenjualan)}</span>
                    </div>

                    {/* Potongan */}
                    <div>
                        <label className="mb-1 block text-sm font-medium text-gray-700">Total Potongan Biaya Kebun (Rp)</label>
                        <input type="number" step="0.01" min="0" value={form.data.total_potongan}
                            ...
                            className={`w-full rounded-md border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-600 ${
                                form.errors.total_potongan || danaTidakValid ? 'border-red-400 bg-red-50' : 'border-gray-300'
                            }`} />
                        {form.errors.total_potongan && <p className="mt-1 text-xs text-red-600">{form.errors.total_potongan}</p>}
                    </div>

                    {/* Dana bersih (otomatis) */}
                    <div className="rounded-lg bg-gray-50 px-3 py-2 text-sm">
                        <div className="flex justify-between">
                            <span className="text-gray-500">Dana Bersih untuk Anggota:</span>
                            <span className={`font-semibold ${danaTidakValid ? 'text-red-600' : 'text-green-700'}`}>
                                {rupiah(danaBersih)}
                            </span>
                        </div>
                        {danaTidakValid
                            ? <p className="mt-1 text-xs text-red-600">Potongan terlalu besar — dana bersih nol/negatif.</p>
                            : <p className="mt-0.5 text-[11px] text-gray-400">Total penjualan − potongan. Otomatis jadi kas keluar.</p>}
                    </div>

                    {/* Tanggal */}
                    <div>
                        <label className="mb-1 block text-sm font-medium text-gray-700">Tanggal Penyaluran</label>
                        <input type="date" value={form.data.tanggal_penyaluran}
                            onChange={(e) => form.setData('tanggal_penyaluran', e.target.value)}
                            className={`w-full rounded-md border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-600 ${
                                form.errors.tanggal_penyaluran ? 'border-red-400 bg-red-50' : 'border-gray-300'
                            }`} />
                        {form.errors.tanggal_penyaluran && <p className="mt-1 text-xs text-red-600">{form.errors.tanggal_penyaluran}</p>}
                    </div>
