                    ...
                    {TABS.map((t) => (
                        <button key={t.key} onClick={() => ubahFilter({ jenis: t.key })}
                            className={`rounded-md px-3 py-1.5 font-medium transition ${
                                filter.jenis === t.key ? 'bg-white text-green-700 shadow-sm' : 'text-gray-500'
                            }`}>
                            {t.label}
                        </button>
                    ))}
                    ...
                    <tbody>
                        {transaksi.data.length === 0 ? (
                            ...
                            transaksi.data.map((t) => (
                                <tr key={t.id_transaksi} className="border-b border-gray-50 last:border-0">
                                    <td className="py-3 text-gray-500">{tanggalID(t.tanggal_transaksi)}</td>
                                    <td className="py-3">
                                        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                                            t.jenis_kas === 'masuk' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'
                                        }`}>
                                    ...
                                    <td className={`py-3 text-right font-semibold ${
                                        t.jenis_kas === 'masuk' ? 'text-green-600' : 'text-red-600'
                                    }`}>
