    ...
    <MainLayout>
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link href="/pemilik/dashboard">
          <button className="p-2 rounded-xl border border-gray-200 hover:bg-gray-50 transition dark:border-gray-800 dark:hover:bg-gray-800">
            <ArrowLeft size={16} className="text-gray-500 dark:text-gray-400" />
          </button>
        </Link>
        <div>
          <h1 className="text-lg font-semibold text-gray-800 dark:text-gray-100">Data Anggota</h1>
          <p className="text-[11px] text-gray-400 dark:text-gray-500">KUD Lubuk Karya</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          ...
          <div key={label} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm text-center dark:bg-gray-900 dark:border-gray-800">
            <p className="text-2xl font-bold" style={{ color }}>{value}</p>
            <p className="text-[11px] text-gray-400 mt-1 dark:text-gray-500">{label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1">
          {isWaiting ? (
            <Loader2 size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 animate-spin dark:text-gray-500" />
          ) : (
          ...
          <input
            ...
            className="w-full pl-8 pr-4 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1B8A3A]/20 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100 dark:placeholder-gray-500"
          />
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-800">
                  <th className="text-left text-[11px] text-gray-400 font-medium px-4 py-3 dark:text-gray-500">No. Surat</th>
                  <th className="text-left text-[11px] text-gray-400 font-medium px-4 py-3 dark:text-gray-500">NIK</th>
                  <th className="text-left text-[11px] text-gray-400 font-medium px-4 py-3 dark:text-gray-500">Nama Lengkap</th>
                  <th className="text-left text-[11px] text-gray-400 font-medium px-4 py-3 dark:text-gray-500">Alamat</th>
                  <th className="text-left text-[11px] text-gray-400 font-medium px-4 py-3 dark:text-gray-500">No. Telepon</th>
                  <th className="text-left text-[11px] text-gray-400 font-medium px-4 py-3 dark:text-gray-500">Tgl. Daftar</th>
                  <th className="text-left text-[11px] text-gray-400 font-medium px-4 py-3 dark:text-gray-500">Tgl. Verifikasi</th>
                  <th className="text-left text-[11px] text-gray-400 font-medium px-4 py-3 dark:text-gray-500">Status</th>
                </tr>
              </thead>
              <tbody>
                {dataAnggota.map((a, i) => (
                  <tr key={a.id_anggota} className={`border-b border-gray-50 hover:bg-gray-50 transition dark:border-gray-800 dark:hover:bg-gray-800 ${i % 2 === 0 ? "" : "bg-gray-50/30 dark:bg-gray-800/30"}`}>
                    <td className="px-4 py-3 text-[11px] text-gray-500 dark:text-gray-400">{a.no_surat_permohonan || "-"}</td>
                    <td className="px-4 py-3 text-[11px] text-gray-600 dark:text-gray-300">{a.nik}</td>
                    <td className="px-4 py-3 text-[12px] font-medium text-gray-800 dark:text-gray-100">{a.nama_lengkap}</td>
                    <td className="px-4 py-3 text-[11px] text-gray-500 max-w-[150px] truncate dark:text-gray-400">{a.alamat || "-"}</td>
                    <td className="px-4 py-3 text-[11px] text-gray-500 dark:text-gray-400">{a.no_telepon || "-"}</td>
      ...
      {/* FOOTER COUNT */}
      <p className="text-[11px] text-gray-400 mt-3 text-right dark:text-gray-500">
