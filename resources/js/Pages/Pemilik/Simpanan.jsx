...
export default function Simpanan() {
    ...
    // Daftar tahun-tahun sebelumnya secara dinamis
    ...
    const paginatedAnggota  = useMemo(() =>
        (perAnggota || []).slice((pageAnggota - 1) * ROWS_PER_PAGE, pageAnggota * ROWS_PER_PAGE),
        [perAnggota, pageAnggota]
    );
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            fetchAnggota({ cari_anggota: cariAnggota });
        }, DEBOUNCE_MS);
    ...
    // ── Shortcut tanggal (dipakai di filter riwayat DAN modal export) ────────
    const getShortcut = (label) => {
        const n = new Date();

        // Cek apakah label adalah angka tahun (misal "2023", "2022", dst)
        if (/^\d{4}$/.test(label)) {
            return getYearRange(Number(label));
        }
    ...
    useEffect(() => {
        ...
        fetch(
            route('pemilik.simpanan') +
            `?start=${exportStart}&end=${exportEnd}&jenis=${jenis}&cari_riwayat=${encodeURIComponent(cariRiwayat)}&preview_count=1`,
    ...
    const handleExportConfirm = async () => {
        ...
        try {
            const res = await fetch(
                route('pemilik.simpanan') +
                `?start=${exportStart}&end=${exportEnd}&jenis=${jenis}&cari_riwayat=${encodeURIComponent(cariRiwayat)}`,
    ...
    const doExportRiwayatCSV = (data) => {
        const ws = XLSX.utils.json_to_sheet(data.map(d => ({
            Tanggal: d.tanggal, Jenis: labelJenis[d.jenis] || d.jenis, Anggota: d.anggota,
            Jumlah: d.jumlah, Keterangan: d.keterangan,
        })));
    ...
    const doExportRiwayatDocx = async (data) => {
        const table = new Table({
            rows: [
                new TableRow({
                    children: ["Tanggal", "Jenis", "Anggota", "Jumlah", "Keterangan"].map(
                        (h) => new TableCell({ children: [new Paragraph(h)] })
                    ),
                }),
                ...data.map((d) =>
                    new TableRow({
                        children: [d.tanggal, labelJenis[d.jenis] || d.jenis, d.anggota, `Rp ${formatRp(d.jumlah)}`, d.keterangan].map(
    ...
    const doExportAnggota = (type) => {
        ...
        if (type === "csv" || type === "excel") {
            const ws = XLSX.utils.json_to_sheet(data.map(a => ({
                Nama: a.nama_lengkap, Pokok: a.pokok, Wajib: a.wajib,
                Pengambilan: a.pengambilan, Total: a.total,
            })));
            if (type === "csv") {
        ...
        if (type === "docx") {
            (async () => {
                const table = new Table({
                    rows: [
                        new TableRow({
                            children: ["Nama", "Pokok", "Wajib", "Pengambilan", "Total"].map(
                                (h) => new TableCell({ children: [new Paragraph(h)] })
                            ),
                        }),
                        ...data.map((a) =>
                            new TableRow({
                                children: [a.nama_lengkap, `Rp ${formatRp(a.pokok)}`, `Rp ${formatRp(a.wajib)}`, `Rp ${formatRp(a.pengambilan)}`, `Rp ${formatRp(a.total)}`].map(
                                    (t) => new TableCell({ children: [new Paragraph(String(t))] })
                                ),
                            })
                        ),
        ...
        <MainLayout>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white rounded-2xl p-4 border border-gray-100 border-l-4 border-l-gray-700 shadow-sm">
                    ...
                    <p className="text-[10px] text-gray-400">Dana Masuk: <span className="text-gray-600">Rp {formatRp(danaMasuk)}</span></p>
                    <p className="text-[10px] text-gray-400">Dana Keluar: <span className="text-gray-600">Rp {formatRp(danaKeluar)}</span></p>
                </div>
                {["pokok", "wajib", "pengambilan"].map((j) => (
                    <div key={j} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm"
                        style={{ borderLeft: `4px solid ${colorJenis[j]}` }}>
                        <p className="text-[11px] text-gray-400 mb-1">Simpanan {labelJenis[j]}</p>
                        <p className="text-xl font-semibold text-gray-800 mb-2">Rp {formatRp(ringkasanJenis[j])}</p>
                        <p className="text-[10px] text-gray-400">
                            {(breakdownJenis || []).find(b => b.jenis === j)?.jumlah_transaksi || 0} transaksi
                        </p>
                    </div>
                ))}
            </div>
                ...
                <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        <div className="relative">
                            ...
                            <input type="text" placeholder="Cari nama anggota..." value={cariAnggota}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 rounded-t-none p-6">
                ...
                <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        <div className="relative">
                            <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
                            <input type="text" placeholder="Cari nama / keterangan..." value={cariRiwayat}
                ...
                <div className="flex flex-wrap items-center gap-2 mb-5 pb-5 border-b border-gray-100">
                    ...
                    {["Bulan Ini", "3 Bulan", "6 Bulan", "Tahun Ini", "Semua Data"].map((label) => (
                        <ShortcutBtn key={label} label={label} isYear={false}
                    ...
                    {/* Tahun-tahun sebelumnya (dinamis) */}
                    {tahunSebelumnya.map((year) => (
                        <ShortcutBtn key={year} label={String(year)} isYear={true}
                            onClick={() => applyDateShortcut(String(year))} />
                    ))}
                                ...
                                {paginated.map((row, i) => (
                                    <tr key={i} className="hover:bg-gray-50">
                                        ...
                                        <td className="py-3 px-4 text-right font-medium">
                                            <span className={row.jenis === "pengambilan" ? "text-red-500" : "text-emerald-600"}>
                                                {row.jenis === "pengambilan" ? "-" : "+"} Rp {formatRp(row.jumlah)}
                                            </span>
                                        </td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                    <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                            {tahunSebelumnya.map((year) => (
                                <ShortcutBtn key={year} label={String(year)} isYear={true}
                                    onClick={() => { const [s, e] = getShortcut(String(year)); setExportStart(s); setExportEnd(e); }} />
                            ))}
                        </div>
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <button onClick={handleExportConfirm}
                                disabled={exportLoading || exportPreviewCount === 0}
